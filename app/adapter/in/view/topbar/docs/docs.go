...
func init() {
	einar.EmbeddedPatterns = append(einar.EmbeddedPatterns, einar.EmbeddedPattern{
		Content: html,
		Pattern: component.Docs + component.DOT_HTML,
	})
	container.InjectInboundAdapter(func() error {
		einar.Echo.GET("/"+component.Docs, render)
		return nil
...
func render(c echo.Context) error {
	routerState := einar.NewRoutingState(c, map[string]string{
		component.TopbarComponentDefault: component.Docs,
	})
	if c.Request().Header.Get(component.FlatContext) != "" {
		return c.Render(http.StatusOK, component.Docs+component.DOT_HTML, routerState)
	}
